...
def write_to_file(msg):
    print(msg)
    with open(file_name, 'a') as fpt:
        fpt.write(msg + "\n")


write_to_file(f"Intermediate width: {intermediate_width}, activation: {activation}, loss function: {loss_function}, batch size: {batch_size}, learning rate: {learning_rate}, use novelty: {use_novel}, gpt frozen: {gpt_frozen}, epochs: {nEpochs}")
write_to_file(f"Train steps per epoch: {len(train_generator)}")

for epoch in range(nEpochs):
    write_to_file(f"XXXXXXXXXXXXXXXXXXXXX Epoch {epoch+1}/{nEpochs}")
    running_loss = 0.0
    running_rmse = 0.0
    model.train()
    for i in range(len(train_generator)):
        ...
        if i % 100 == 99:
            write_to_file(f"epoch: {epoch+1}, step: {i+1}, rmse_loss: {float(rmse):.8f}, {loss_function}_loss {float(loss):.8f}")
    ...
    with torch.no_grad():
        ...
        write_to_file(f"epoch: {epoch+1}, train rmse_loss: {float(train_rmse):.8f}, train {loss_function}_loss {float(train_loss):.8f}, eval rmse_loss: {float(eval_rmse):.8f}, eval {loss_function}_loss {float(eval_loss):.8f}")
